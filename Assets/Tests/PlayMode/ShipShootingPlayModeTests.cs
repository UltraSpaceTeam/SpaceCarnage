...
/// <summary>
/// PlayMode тесты для ShipShooting.
/// [Command] / [ClientRpc] не вызываются напрямую — тестируется внутренняя логика.
/// ShipAssembler и Player добавляются как реальные компоненты, нужные поля мокаются рефлексией.
/// </summary>
public class ShipShootingPlayModeTests
{
    ...
    public IEnumerator SetUp()
    {
        ...
        _go.SetActive(false); // Awake не вызывается пока неактивен
        ...
        // Инжектируем _assembler до Awake
    ...
    /// <summary>Создаёт WeaponData с заданными параметрами.</summary>
    ...
    /// <summary>Мокает CurrentWeapon в ShipAssembler через property с private set.</summary>
    ...
    /// <summary>Мокает CurrentWeaponObject в ShipAssembler через property с private set.</summary>
    ...
    {
        // _player == null ? ShooterName должен вернуть "Unknown"
    ...
    {
        // CurrentWeapon по умолчанию null — просто проверяем что не падает
    ...
    public IEnumerator ReloadCoroutine_SetsIsReloadingTrue_DuringReload()
    {
        ...
        yield return null; // один кадр — перезарядка идёт
    ...
    public IEnumerator ReloadCoroutine_RestoresAmmo_AfterDelay()
    {
        ...
        yield return new WaitForSeconds(0.2f); // ждём дольше reload
    ...
    /// <summary>Запускает приватную корутину через рефлексию.</summary>
    ...
    {
        // CurrentWeaponObject == null ? _muzzlePoint должен стать transform корабля
    ...
    {
        // Создаём WeaponObject с дочерним объектом "Muzzle"
    ...
    public IEnumerator Awake_LaserBeamRenderer_DisabledByDefault()
    {
        ...
        // Если LineRenderer есть — должен быть отключён
        if (lr != null)
        {
